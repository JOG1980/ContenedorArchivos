...
function loader(show) {
  if (show) $("#wait").css("display", "block");
...
function armarResultadosContenidoDetalles(contenedor_items, items) {
  let contenido_items =
    "<table id='table_items_view' class='table' style='width: 100%;'>" +
  ...
  for (i = 0; i < items.length; i++) {
    ...
    if (tipo == "file") {
      ...
      let image_name = "file_extension_" + ext + ".png";
      contenido_items +=
        "<tr TIPO='file'><td><input type='checkbox' name='checkbox_" +
        contador +
        "'></td>" +
        "<td>" +
      ...
      contenido_items +=
        "<tr  TIPO='folder'><td><input type='checkbox' name='checkbox_" +
        contador +
        "'></td>" +
        "<td>" +
...
function initButtonsOn() {
  ...
  $(document).on("click", "#btn_folders_show", function (event) {
    ...
    if (hay_nodos_abiertos) {
      ...
      //si no hay nada abierto, los abrimos
      tree.open_all();
    }
  });
  ...
  $(document).on(
    "click",
    "#btn_folder_add , #btn_folder_delete , #btn_folder_renombrar , #btn_file_delete , #btn_file_renombrar",
    function (e) {
      let ruta_actual = $("#hd_ruta_actual").val();
